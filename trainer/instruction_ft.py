...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask, feature_attention_mask, padding_mask, input_features) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            ...
            # clean_state_dict = {k: v.half() for k, v in clean_state_dict.items()}  # 存在bug 半精度保存 残留autograd导致显存不断上涨直到溢出
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../dataset/voice_assistant")
